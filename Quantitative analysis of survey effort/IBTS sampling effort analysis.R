...
#ns_old <- read_sf('C:/Users/astroh/OneDrive - Marine Institute/Chapter 1/Data/ICES_Statistical_Rectangles_Eco.shp') |>
 # filter(Ecoregion %in% c("Greater North Sea")) |>
  #st_drop_geometry()
#names(ns_old)

ns <- read_sf(paste0(file_path, "/", "ICES_StatRec_mapto_ICES_Areas/StatRec_map_Areas_Full_20170124.shp")) |>
  filter(Area_27 %in% c("4.a", "4.b", "4.c", "7.d", 
                        "3.a.20", "3.a.21", "3.c.22")) |>
  select(ICESNAME, Area_27, SOUTH, WEST, NORTH, EAST, geometry) |>
  #merge(ns_old) |>
...
q1_with_miss_sf <- ns |> # Make the final dataframe an sf object
  ...
  st_write(paste0(output_path, "surveyeffort_q1_1998-2023.shp"), append = F)
...
#### CLEAR ENVIRONMENT BEFORE RUNNING CODE BELOW ####

# Calculate mean relative sampling effort by manual cycle -----------------

# Quarter 1
q1 <- read_sf(paste0(output_path, "/", "Created files/surveyeffort_q1_1998-2023.shp"))
...
# Quarter 3
q3 <- read_sf(paste0(output_path, "/", "surveyeffort_q3_1998-2023.shp"))
...
# Summary statistics of relative sampling effort ---------------------

# Quarter 1
names(q1)
mean(q1$PercDev) # -10.45361
sd(q1$PercDev) # 49.38872
min(q1$PercDev) # -100
max(q1$PercDev) # 350
# 350 extreme - check row
q1[q1$PercDev == max(q1$PercDev), ] # 9 hauls in one rec on 2010

# Quarter 3
names(q3)
mean(q3$PercDev) # -13.97238
sd(q3$PercDev) # 46.17989
min(q3$PercDev) # -100
max(q3$PercDev) # 250
...
# Load bathymetry
bathymetry <- read_sf(paste0(output_path, "/", "bathymetry shallower than 10m.shp")) |>
  st_transform(23032)

# Load geographic context
canvas <- read_sf("C:/Users/astroh/OneDrive - Marine Institute/Chapter 1/Plotting canvases/North Sea map.shp") |>
  st_transform(23032)

q1_summary <- ggplot() +
  geom_sf(data = q1_mean, aes(fill = MeanPercDev), alpha = 0.7, colour = "black", linewidth = 0.05) +
  geom_sf(data = canvas, fill = "gray40", colour = "black", linewidth = 0.05) +
  geom_sf(data = bathymetry, fill = "gray45") +
  coord_sf(xlim = c(-346881.9, 771558), ylim = c(5506831, 6914779)) +
  scale_fill_gradientn("\nAverage \nrelative \nsampling \neffort (%)", 
                       values=scales::rescale(c(-100, -50, 0, 50, 100, 150)), 
                       colors = c("#440154", "#481567", "white", "#55C997", "#95D840", "#FDE725")) +
  guides(fill = guide_colorbar(barwidth = 0.7,
                               frame.colour = "grey65", 
                               frame.linewidth = 0.2)) +
  #scale_fill_gradientn("\nAverage \nrelative \nsampling \neffort (%)", 
                      # values=scales::rescale(c(-100, -50, 0, 50, 100, 150)), 
                      # colors = c("#440154", "#453781", "#3CBB75", "#95D840", "#B8DE29", "#DCE319")) +
  #scale_fill_gradientn("\nAverage \nrelative \nsampling \neffort (%)", 
   #                    values=scales::rescale(c(-100, -50, 0, 50, 100, 150)), 
  ...
  theme(panel.grid.major = element_blank(),
        ...
        strip.text = element_text(size = 6.5, face = "bold", margin = margin(2, 2, 2, 2)),
...
library(patchwork)
ibts_patch <- q1_summary + q3_summary + 
  plot_layout(guides = "collect") + 
  plot_annotation(tag_levels = "A") &
  theme(plot.tag = element_text(size = 7))
print(ibts_patch)

ggsave("ibts_avrelsampeff2.png", ibts_patch, 
       width = 170, height = 75, units = "mm", dpi = 1000)
